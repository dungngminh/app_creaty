...
class _FlutterWidgetState extends material.State<FlutterWidget> {
  ...
  material.Widget build(material.BuildContext context) {
    ...
    if (widget.assetPath != null) {
      return material.FutureBuilder(
        future: services.rootBundle
            ...
            .then(jsonDecode)
            .then(
              (value) => widgets.Widget.fromJson(value as Map<String, Object?>),
            ),
    ...
    if (widget.request != null) {
      ...
      if (!req.streamedResponse) {
        return material.FutureBuilder(
          future:
              $request(req).then((value) => value.body).then(jsonDecode).then(
                    (value) =>
                        widgets.Widget.fromJson(value as Map<String, Object?>),
                  ),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              final widget = snapshot.data!;
  ...
  Future<http.Response> $formSubmit(
    ...
    final formJson = json.encode(_formData.value.toJsonMap());
    final request0 = widgets.NetworkHttpRequest(
      method: 'POST',
      url: request.url,
      headers: {
        ...request.headers,
  ...
  material.VoidCallback? $callback(
    ...
    return callback.maybeMap(
      ...
      clearMaterialBanners: (_) => messengerKey.clearMaterialBanners,
      hideCurrentMaterialBanner: (_) => messengerKey.hideCurrentMaterialBanner,
            ...
            onChanged: (val) {
              if (value.field != null) {
                ...
                if (fieldState is widgets.FormBoolField) {
                  _formData.value = formData.updateField(
                    field.copyWith(
                      value: val,
                    ),
                  );
